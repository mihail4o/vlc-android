...
if [ $ON_WINDOWS -eq 1 ] ; then
    NDK_LIB_DIR=`cygpath -m $NDK_LIB_DIR`
fi
...
if [ $ON_WINDOWS -eq 1 ]; then
    OSCMD=.cmd
    VLC_SRC_DIR=`cygpath -m $VLC_SRC_DIR`
fi

INC_VLC_MODULES="$VLC_SRC_DIR/$VLC_BUILD_DIR/vlc_modules.mk"
INC_VLC_CONTRIB_LDFLAGS="$VLC_SRC_DIR/$VLC_BUILD_DIR/vlc_contrib_ldflags.mk"

echo "# Autogenerated, DO NOT EDIT" > $INC_VLC_MODULES
echo "LOCAL_LDLIBS += \\" >> $INC_VLC_MODULES
for i in ${VLC_MODULES}
do
    if [ $ON_WINDOWS -eq 1 ]; then
        echo "  `cygpath -m $i` \\" >> $INC_VLC_MODULES
    else
        echo "  $i \\" >> $INC_VLC_MODULES
    fi
done
echo "" >> $INC_VLC_MODULES

echo "# Autogenerated, DO NOT EDIT" > $INC_VLC_CONTRIB_LDFLAGS
echo "LOCAL_LDLIBS += \\" >> $INC_VLC_CONTRIB_LDFLAGS
for i in ${VLC_CONTRIB_LDFLAGS}
do
    case $i in
        -L*)
            [ $ON_WINDOWS -eq 1 ] || echo "  -L${i#-L} \\" >> $INC_VLC_CONTRIB_LDFLAGS
            ;;
        /c/*)
            [ $ON_WINDOWS -eq 1 ] || echo "  $i \\" >> $INC_VLC_CONTRIB_LDFLAGS
            ;;
        *)
            echo "  $i \\" >> $INC_VLC_CONTRIB_LDFLAGS
            ;;
    esac
done
echo "" >> $INC_VLC_CONTRIB_LDFLAGS

if [ $ON_WINDOWS -eq 1 ]; then
    VLC_SRC_DIR=`cygpath -m $VLC_SRC_DIR`
    VLC_BUILD_DIR=`cygpath -m $VLC_BUILD_DIR`
    VLC_CONTRIB=`cygpath -m $VLC_CONTRIB`
fi
...
$ANDROID_NDK/ndk-build$OSCMD -C libvlc \
    ...
    INC_VLC_CONTRIB_LDFLAGS="$INC_VLC_CONTRIB_LDFLAGS" \
    INC_VLC_MODULES="$INC_VLC_MODULES" \
